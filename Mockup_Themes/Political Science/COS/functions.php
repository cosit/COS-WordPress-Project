...
/** Tell WordPress to update the RSS Widget every hour **/
...
//     wp_register_script('jquery.derp', 'https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js', false, '1.7.1');
//     wp_enqueue_script('jquery.derp');
    ...
    <div class="wrap">  
        ...
        <form method="POST" action="">  
            ...
            <input type="text" name="events_items" id="events_items" value=<?php echo $events_items; ?>>
  			...
  			<?php /*All things related to news/events will not be here anymore 
To be put in widget options (so plugins will need to be updated)*/
			?>
...
function page_nav( $pageID = 0 ){
	...
	//If pageID equals the pageID for NEWS then export a list of the 
	//News Archives
	if( $title == 'News' ){
		echo '<ul>';
...
function people_nav( $pageID = '' ){
	...
	echo '<nav class="pageNav"><h2><a href="'.get_bloginfo('url').'/people/">People</a></h2><ul>';
	...
	if(have_posts()) : while (have_posts()) : the_post();			
		...
		$contact = array(
			...
			'email'      => '<a href="mailto:'.get_field('email').
				'">'.get_field('email').'</a>',
			'email2'      => '<a href="mailto:'.get_field('email2').
				'">'.get_field('email2').'</a>',
			'fax'        => 'F: ' . get_field('fax'),
