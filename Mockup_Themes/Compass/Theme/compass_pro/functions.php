...
function show_person( $id, $is_ie = false ) {
	...
	$personBasics = array(
		...
		'person_email' => antispambot(sanitize_email(get_field('email'))),
		'person_cv' => get_field('curriculum_vitae'),
		
	);
	$cvLink = get_field('cv_link');
	if(!empty($cvLink))
		$personBasics['person_cv'] = $cvLink ;
	...
	$person = array(
		...
		'p_office_hours_by_appointment' => get_field('office_hours_by_appointment_only'),

	);

	if(!$person['p_photo'])
		$person['p_photo'] = "http://psychology.cos.ucf.edu/main/files/uploads/2012/08/NoImagePegasusGrey.jpg";
...
function get_hrs( $person ){
	...
	$appointment_msg = '<br/>Office hours are by appointment only.  Please contact '.$person['p_first_name'].' '.$person['p_last_name'].' to schedule an appointment<br/><br/>';
	
	//If the By Appointment only checkbox has been selected then the appropriate messge is returned
	if($person['p_office_hours_by_appointment'] )
	...
	foreach( $days as $day => $dayTitle ){
		...
		if( is_array($person[$office_hours_today]) ){
			...
			$emptyHours = "Not Empty";
		} else {
			if(strpos($person[$office_hours_today], 'ppointment') !== false ) $hours .= '<em> By Appointment </em>';
			else
...
function starkers_widgets_init() {
	...
	register_sidebar( array(
		...
		'before_widget' => '',
		'after_widget' => '<br/><br/>',
		'before_title' => '<h2><span class="contact_title">',
		'after_title' => '</span></h2>',
	) );
